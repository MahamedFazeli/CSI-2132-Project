...
<body>
    ...
    <form method="POST" id="search-form">
        ...
        <input type="text" id="hotel-chain" name="hotel-chain">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$result || mysqli_num_rows($result) === 0) {
    ...
    echo "<table>";
    echo "<tr><th>Hotel ID</th><th>Room number</th><th>Capacity</th><th>Price</th><th>View</th><th>Book</th></tr>";
    while ($row = mysqli_fetch_assoc($result)) {
      echo "<tr><td>" . $row['hotel_ID'] . "</td><td>" . $row['room_ID'] . "</td><td>" . $row['Capacity'] . "</td><td>" . $row['Price'] ."</td><td>" . $row['view_type'] . "</td><td><form method='POST'><input type='hidden' name='room_id' value='" . $row['room_ID'] . "'><button type='submit'>Book</button></form></td></tr>";
    }
    echo "</table>";
  }
}
